...
const Customers = ({ url, token }) => {
  ...
  const authToken = token || (typeof window !== 'undefined' ? window.localStorage.getItem('token') : '');
  const storeId = typeof window !== 'undefined' ? window.localStorage.getItem('storeId') : '';

  const fetchCustomers = async (page = 1, search = '') => {
    setLoading(true);
    try {
      const response = await axios.get(`${url}/api/customers/store/${storeId}`, {
        headers: { Authorization: `Bearer ${authToken}` },
        params: { page, search, limit: 10 }
      });

      if (response.data.success) {
        const list = response.data.customers || response.data.data?.customers || [];
        const pages = response.data.pagination?.total || response.data.data?.pagination?.totalPages || 1;
        setCustomers(list);
        setTotalPages(pages);
        setCurrentPage(page);
      } else {
  ...
  const fetchAutoRegisterCustomers = async (page = 1, search = '') => {
    setLoading(true);
    try {
      const response = await axios.get(`${url}/api/customer-auto/admin/customers`, {
        headers: { Authorization: `Bearer ${authToken}` },
        params: { page, search, limit: 10, storeId }
      });

      if (response.data.success) {
        const list = response.data.customers || response.data.data?.customers || [];
        const pages = response.data.pagination?.total || response.data.data?.pagination?.totalPages || 1;
        setAutoRegisterCustomers(list);
        setTotalPages(pages);
        setCurrentPage(page);
      } else {
        toast.error(response.data.message);

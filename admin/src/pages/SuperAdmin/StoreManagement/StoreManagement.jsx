...
import { SUPPORTED_LANGUAGES, SUPPORTED_CURRENCIES, SUPPORTED_COUNTRIES, getRegionalDefaults, getRegionalDefaultsByCountry } from '../../../constants/localization';
import { getSuperAdminToken } from '../../../utils/auth';

const StoreManagement = ({ url, token }) => {
  // Use sempre o token do Super Admin para rotas do sistema;
  // faz fallback para o token passado por props caso o superAdminToken não exista.
  ...
  // Função para atualizar país e ajustar idioma/moeda/fuso automaticamente
  const handleCountryChange = (e) => {
    const selectedCountry = e.target.value;
    const defaults = getRegionalDefaultsByCountry(selectedCountry);
    setFormData(prev => ({
      ...prev,
  ...
  const fetchStores = async () => {
    ...
    try {
      ...
      // Validação de formato do token (JWT deve ter 3 partes)
      if (typeof authToken !== 'string' || authToken.split('.').length !== 3) {
        console.error('Token inválido ou malformado:', authToken);
        toast.error('Token inválido. Faça login novamente como Super Admin.');
        setLoading(false);
        return;
      }
      ...
      if (response.data.success) {
        const rawStores = response.data?.data?.stores || [];
        const normalizedStores = rawStores.map((store) => ({
          ...store,
          ownerName: store.owner?.name || store.ownerName || '',
          ownerEmail: store.owner?.email || store.ownerEmail || store.email || '',
          subscriptionPlan: store.subscription?.plan || store.subscriptionPlan || 'Básico',
          restaurantAddress: store.settings?.restaurantAddress || store.restaurantAddress || '',
          language: store.settings?.language || store.language || 'pt-BR',
          currency: store.settings?.currency || store.currency || 'BRL',
          timezone: store.settings?.timezone || store.timezone || 'America/Sao_Paulo',
        }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Exibir senha temporária gerada pelo backend, se existir
        const tempPassword = response?.data?.data?.tempPassword;
        if (!editingStore && tempPassword) {
          toast.info(`Senha temporária do proprietário: ${tempPassword}`);
          console.log('Senha temporária do proprietário:', tempPassword);
        }
        resetForm();
        fetchStores();
      } else {
      ...
      if (backendMessage && backendMessage.toLowerCase().includes('token')) {
        toast.error('Token inválido. Faça login novamente como Super Admin.');
      } else {
        toast.error(backendMessage || 'Erro ao conectar com o servidor');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleEdit = (store) => {
    ...
    setFormData({
      ...
      ownerEmail: store.ownerEmail || store.owner?.email || store.email || '',
      ownerPassword: '', // Não pré-preencher senha por segurança
      subscriptionPlan: store.subscription?.plan || store.subscriptionPlan || 'Básico',
      country: store.settings?.address?.country || 'Brasil',
      language: store.settings?.language || store.language || 'pt-BR',
      currency: store.settings?.currency || store.currency || 'BRL',
      timezone: store.settings?.timezone || store.timezone || 'America/Sao_Paulo',
      // Campos do Telegram
  ...
  const toggleStoreStatus = async (storeId, currentStatus) => {
    ...
    try {
      ...
      if (response.data.success) {
        toast.success(`Loja ${newStatus === 'active' ? 'ativada' : 'inativada'} com sucesso!`);
        fetchStores();
      } else {
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className='form-group'>
                ...
                <input
                  ...
                  placeholder={editingStore ? 'Deixe em branco para manter a senha' : ''}
                />
              </div>
              ...
              <div className='form-group'>
                ...
                <small>Escolha o país; idioma, moeda e fuso serão ajustados</small>
              </div>

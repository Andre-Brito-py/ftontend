...
class CustomerAutoRegister {
    ...
    async loadExistingCustomer(clientId) {
        try {
            const response = await fetch(`${this.apiBaseUrl}/customer-auto/register`, {
                method: 'POST',
                headers: {
    ...
    showRegistrationModal() {
            ...
            <div id="customerRegistrationModal" class="customer-modal-overlay">
                <div class="customer-modal">
                    <div class="customer-modal-header">
                        <h3>Bem-vindo!</h3>
                        <p>Para facilitar seus próximos pedidos, precisamos do seu nome e número de telefone.</p>
                    </div>
                    ...
                    <div class="customer-modal-body">
                        <div class="customer-form-group">
                            ...
                            <input 
                                ...
                                required
                            >
                            <div id="nameError" class="customer-error-message"></div>
                        </div>
                        <div class="customer-form-group">
        ...
        setTimeout(() => {
            ...
            (nameEl || document.getElementById('customerPhone')).focus();
        }, 100);
    }
    ...
    addModalStyles() {
        ...
        const styles = `
            <style id="customerModalStyles">
                ...
                .customer-form-group input[type="text"]:focus {
                    outline: none;
                    border-color: #3b82f6;
                    box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.1);
                }
    ...
    setupModalEvents() {
        ...
        const updateConfirmButton = () => {
            ...
            const nameValid = this.isValidName((nameInput?.value || '').trim());
            const consentGiven = lgpdCheckbox.checked;
            
            confirmBtn.disabled = !phoneValid || !nameValid || !consentGiven;
        };

        phoneInput.addEventListener('input', updateConfirmButton);
        if (nameInput) nameInput.addEventListener('input', updateConfirmButton);
    ...
    validateName() {
        ...
        if (!name) {
            this.showNameError('Nome é obrigatório');
            return false;
        }
        if (name.length < 2) {
            this.showNameError('Informe pelo menos 2 caracteres');
            return false;
        }
        this.hideNameError();
        return true;
    ...
    showWelcomeMessage(customerData) {
        ...
        notification.innerHTML = `
            <div style="font-weight: 600; margin-bottom: 4px;">
                ${name ? `Olá, ${name}! ` : ''}Cadastro realizado com sucesso!
            </div>
    ...
    async requestDataDeletion() {
        ...
        try {
            const response = await fetch(`${this.apiBaseUrl}/customer-auto/customer/${this.clientData.clientId}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                },

...
const Header = () => {
  ...
  useEffect(() => {
    if (url) {
      // Recarrega quando a URL da API ou o storeId mudarem
      fetchBanners();
    }
  }, [url, storeId]);
  ...
  const bannerImage = currentBanner.image || '';
  const hasValidImage = typeof bannerImage === 'string' && bannerImage.length > 0;
  const baseBannerUrl = hasValidImage
    ? ((bannerImage.startsWith('/'))
        ? `${url}${bannerImage}`
        : `${url}/images/${bannerImage}`)
    : null;
  const backgroundImage = baseBannerUrl
    ? `${baseBannerUrl}${storeId ? `?storeId=${storeId}` : ''}`
    : assets.header_img;

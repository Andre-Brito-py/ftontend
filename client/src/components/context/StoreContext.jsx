...
const StoreContextProvider = (props) => {
    ...
    // Base da API: em dev usar proxy do Vite (paths relativos), em produção usar VITE_BACKEND_URL
    const url = (import.meta.env?.DEV ? '' : (import.meta.env.VITE_BACKEND_URL || 'http://localhost:4011'));
    ...
    const loadStoreData = useCallback(async (storeSlug) => {
        try {
            // Evitar hang: usar timeout por requisição e tratar respostas individualmente
            const reqOptions = { timeout: 8000 };
            const promises = [
                axios.get(`${url}/api/store/public/${storeSlug}`, reqOptions),
                axios.get(`${url}/api/store/public/${storeSlug}/menu`, reqOptions)
            ];

            const [storeResult, menuResult] = await Promise.allSettled(promises);

            // Processar dados da loja
            if (storeResult.status === 'fulfilled' && storeResult.value?.data?.success) {
                const data = storeResult.value.data;
                const storeData = data.store || data.data?.store || null;
                if (storeData) {
                    setCurrentStore(storeData);
                    setStoreId(storeData._id || storeData.id || null);
                }
            }

            // Processar menu da loja
            if (menuResult.status === 'fulfilled' && menuResult.value?.data?.success) {
                const data = menuResult.value.data?.data || {};
                setStoreMenu({
            ...
            const success = (storeResult.status === 'fulfilled' && storeResult.value?.data?.success) ||
                            (menuResult.status === 'fulfilled' && menuResult.value?.data?.success);

            if (!success) {
                return { success: false, error: new Error('Falha ao carregar dados ou menu da loja') };
            }
            return { success: true };
        } catch (error) {
